...
export const validate = (rule: ValidationRule) => {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const securityErrors = checkSecurityIssues(req);
...
function validateUploadedFiles(files: any): string[] {
  const errors: string[] = [];
...
function hasMaliciousFilename(filename: string): boolean {
...
function checkSecurityIssues(req: Request): string[] {
  const errors: string[] = [];

  // Check body for SQL injection
  if (req.body) {
    const hasSqlInjection = checkObjectForSqlInjection(req.body);
    if (hasSqlInjection) {
  ...
  if (req.query) {
    const hasSqlInjection = checkObjectForSqlInjection(req.query);
    if (hasSqlInjection) {
