...
exports.visualGeneratorAgent = exports.VisualGeneratorAgent = void 0;
...
class VisualGeneratorAgent {
    ...
    async generateVisuals(requirements, assetTypes = ['diagram', 'ui']) {
        ...
        try {
            ...
            logger_1.default.error('Visual generator agent error:', error);
            throw new Error(`Visual generation failed: ${error.message}`);
        }
    }
    generateVisualPrompt(requirements, assetTypes) {
...
Make the designs professional, clear, and aligned with modern design principles.`;
    ...
    parseVisualResponse(response, assetTypes) {
    ...
    normalizeAssetType(type) {
        ...
        if (normalized.includes('chart') || normalized.includes('graph'))
            return 'chart';
        if (normalized.includes('ui') || normalized.includes('mockup') || normalized.includes('wireframe'))
            return 'ui';
    ...
    async generateArchitectureDiagram(systemDescription) {
...
4. Mermaid.js code for the diagram`;
    ...
    async generateUIMockups(requirements) {
        const result = await this.generateVisuals(requirements, ['ui']);
        return result.assets.filter(asset => asset.type === 'ui');
    }
}
exports.VisualGeneratorAgent = VisualGeneratorAgent;
exports.visualGeneratorAgent = new VisualGeneratorAgent();
