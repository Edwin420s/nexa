...
export class CodeBuilderAgent {
  ...
  private generateProjectPrompt(requirements: string, stack: string): string {
  ...
  private parseProjectResponse(response: string, stack: string): Omit<ProjectStructure, 'confidence' | 'metadata'> {
    const lines = response.split('\n');
    let currentFile: { name?: string; language?: string; content: string[] } | null = null;
        ...
        while (i < lines.length && lines[i].trim() && !lines[i].startsWith('##')) {
          ...
          if (depMatch) {
            dependencies.push({
              ...
              type: depMatch[3].trim().toLowerCase() as 'production' | 'development'
            });
          }
          i++;
        }
  ...
  private detectFileType(filename: string): 'component' | 'utility' | 'config' | 'test' {
    if (filename.includes('.test.') || filename.includes('.spec.')) return 'test';
    if (filename.includes('config') || filename.includes('.env') || filename.includes('.json')) return 'config';
    if (filename.includes('util') || filename.includes('helper') || filename.includes('lib')) return 'utility';
    return 'component';
  }
  ...
  async generateTests(code: string, testFramework: string = 'jest'): Promise<CodeFile[]> {
...
export const codeBuilderAgent = new CodeBuilderAgent();